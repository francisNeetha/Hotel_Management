...
const { addReview } = require("../models/customerModel");
    ...
    customerModel.getAdminByEmail(email, (err, admin) => {
        ...
        if (admin) {
            ...
            customerModel.getCustomerByEmail(email, (err, customer) => {
                ...
                bcrypt.compare(password, customer.password, (err, isValidPassword) => {
                    if (err) {
                        return res.status(500).json({ error: "Error comparing passwords." });
                    }
...
const signup = async (req, res) => {
    try {
        ...
        const token = jwt.sign(
            { id: result.insertId, email: newCustomer.email, role: newCustomer.role,customerId: newCustomer.id, },
            process.env.JWT_SECRET,
            { expiresIn: "1h" }
        );
...
const updateCustomer = (req, res) => {
    ...
    values.push(...[data.name, data.email, data.phone, data.address, data.role].filter(Boolean));
    values.push(id);
    ...
    if (req.user.role === "admin") {
        ...
        customerModel.updateCustomer(query, values, (err, result) => {
            if (err) {
                return res.status(500).json({ error: "Failed to update your data." });
            }
            if (result.affectedRows === 0) {
                return res.status(404).json({ error: "Customer not found." });
            }
...
const getAllBookings = (req, res) => {
    ...
    customerModel.getBookings((err, rows, fields) => {
        if (err || !rows) {
...
const getCustomerBookings = (req, res) => {
    ...
    customerModel.getBookingsByCustomerId(customerId, (err, rows) => {
        if (err || !rows) {
